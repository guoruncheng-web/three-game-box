...
import { Gamepad2, Mail, Lock, Eye, EyeOff, Sparkles, Star, Heart, Zap } from 'lucide-react';
...
export function LoginPage({ onLogin, onShowRegister }: LoginPageProps) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (!phone) {
      showToast('warning', '请输入手机号哦～', '📱');
      return;
    }
    
    if (!password) {
      showToast('warning', '请输入密码哦～', '🔐');
      return;
    }
  ...
  const handleSocialLogin = (platform: string) => {
    showToast('info', `正在跳转到${platform}登录...`, '🔄');
    // 模拟第三方登录成功
    setTimeout(() => {
      showToast('success', `${platform}登录成功！`, '✨');
      setTimeout(() => {
        onLogin();
      }, 500);
    }, 1000);
    ...
    <div className="min-h-screen bg-gradient-to-b from-sky-300 via-purple-300 to-pink-300 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-6 animate-slide-down">
          ...
          <p className="text-white text-base font-medium animate-fade-in">开启你的快乐冒险时光 ✨</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 backdrop-blur-sm relative overflow-hidden animate-slide-up">
          ...
          <form onSubmit={handleLogin} className="space-y-5 relative z-10">
            ...
            {/* 记住我和忘记密码 */}
            <div className="flex items-center justify-between text-sm pt-2 animate-fade-in-up delay-300">
              <label className="flex items-center gap-2 cursor-pointer group">
                <input
                  type="checkbox"
                  checked={rememberMe}
                  onChange={(e) => setRememberMe(e.target.checked)}
                  className="w-5 h-5 rounded-lg border-2 border-purple-300 text-purple-600 focus:ring-purple-500 cursor-pointer transition-transform hover:scale-110"
                />
                <span className="text-gray-600 font-medium group-hover:text-gray-800 transition-colors">记住我 💝</span>
              </label>
              <button
                type="button"
                className="text-purple-600 hover:text-purple-700 font-bold transition-all hover:scale-105 transform"
              >
                忘记密码？
              </button>
            </div>
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-purple-500 via-pink-500 to-orange-500 text-white py-4 rounded-2xl font-black text-lg shadow-xl hover:shadow-2xl hover:scale-[1.05] active:scale-95 transition-all duration-300 relative overflow-hidden group animate-fade-in-up delay-400"
          ...
          <div className="relative my-6 animate-fade-in-up delay-500">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-white text-gray-500 font-bold">快速登录 ⚡</span>
            </div>
          </div>
          ...
          {/* 注册链接 */}
          <div className="mt-6 text-center text-base animate-fade-in-up delay-700">
            <span className="text-gray-600 font-medium">还没有账号？</span>
            <button
              type="button"
        ...
        {/* 底部提示 */}
        <p className="text-center text-white text-xs mt-6 font-medium animate-fade-in delay-800">
          登录即表示同意
          <button className="underline hover:text-yellow-200 transition-colors mx-1 font-bold">用户协议</button>
          和
          <button className="underline hover:text-yellow-200 transition-colors ml-1 font-bold">隐私政策</button>
        </p>
      </div>
